...
ApplicationInit(EResetReason_t eResetReason)
{
  ...
  DPRINT("\n\n--------------------------------\n");
  DPRINT("Z-Wave Sample App: JAQUAR ONOFF DRY CONTACT \n");
...
////             TimerStart(&button_press_timer, 500);
...
//            {
//          DPRINT("CHECK\r\n");
//      //          TimerStart(&learnmode_timer,500);
//              if (EINCLUSIONSTATE_EXCLUDED != g_pAppHandles->pNetworkInfo->eInclusionState)
//              {
//                DPRINT("LEARN_MODE_EXCLUSION\r\n");
//                ZAF_setNetworkLearnMode(E_NETWORK_LEARN_MODE_EXCLUSION_NWE);
...
AppStateManager(EVENT_APP event)
{
  ...
  switch(currentState)
  {
    ...
    case STATE_APP_IDLE:
      ...
      {
        //Board_IndicateStatus(BOARD_STATUS_IDLE);
         //ZAF_EventHelperEventEnqueue(EVENT_APP_BUTTON_LEARN_RESET_SHORT_PRESS);
      }
...
//      if(EVENT_SW1_PRESS==event)
//        {
//
//             DPRINTF("\n\n-----sw1 press\n\n");
//             ToggleLed(ENDPOINT_1);
//
//        }
//      if(EVENT_SW2_PRESS==event)
//        {
//
//             DPRINTF("\n-----sw2 press");
//             ToggleLed(ENDPOINT_2);
//        }
//      if(EVENT_SW3_PRESS==event)
//        {
//
//             DPRINTF("\n-----sw3 press");
//             ToggleLed(ENDPOINT_3);
//        }
//      if(EVENT_SW4_PRESS==event)
//        {
//
//             DPRINTF("\n-----sw4 press");
//             ToggleLed(ENDPOINT_4);
//        }
            if(APP_EVENT_PB_UP == (APP_BUTTON_EVENT)event)
                {
                   //ZAF_EventHelperEventEnqueueFromISR(APP_EVENT_INCLUSION);
      ...
      if (EVENT_APP_BUTTON_TOGGLE_SHORT_PRESS == event)
      {
        ...
        /* Update the lifeline destinations when the Binary Switch state has changed */
//        void * pData = CC_BinarySwitch_prepare_zaf_tse_data(&zaf_tse_local_actuation);
//        ZAF_TSE_Trigger(CC_Binary_Switch_report_stx, pData, true);
      }
...
SetDefaultConfiguration(void)
{
  ...
  ApplicationData.KeypadID = nvm_data_buff[0] = 0;
  ApplicationData.Relay1BtnNo = nvm_data_buff[1] = 0;
  ApplicationData.Relay2BtnNo = nvm_data_buff[2] = 0;
  ApplicationData.Relay3BtnNo = nvm_data_buff[3] = 0;
  ApplicationData.Relay4BtnNo = nvm_data_buff[4] = 0;
  ApplicationData.relay1State = false;
  ApplicationData.relay2State = false;
  ApplicationData.relay3State = false;
  ApplicationData.relay4State = false;
  memset(relay_states, false, 4);
  memset(nvm_data_buff, 0, 5);
  ZAF_Reset();
...
readAppData(void)
{
 ...
 // ASSERT(ZPAL_STATUS_OK == status); //Assert has been kept for debugging , can be removed from production code. This error hard to occur when a corresponing write is successfull
   ...
   {
       nvm_data_buff[0] = AppData.KeypadID;
       nvm_data_buff[1] = AppData.Relay1BtnNo;
       nvm_data_buff[2] = AppData.Relay2BtnNo;
       nvm_data_buff[3] = AppData.Relay3BtnNo;
       nvm_data_buff[4] = AppData.Relay4BtnNo;
       relay_states[0] = AppData.relay1State;
       relay_states[1] = AppData.relay2State;
       relay_states[2] = AppData.relay3State;
       relay_states[3] = AppData.relay4State;

       // UPDATE last_relay_state from NVM data
       last_relay_state[0] = AppData.relay1State;
       last_relay_state[1] = AppData.relay2State;
       last_relay_state[2] = AppData.relay3State;
       last_relay_state[3] = AppData.relay4State;

       DPRINTF("\n======Nvm3 App Read Data Success=======");
       DPRINTF("\nKeypad ID : %d\n", nvm_data_buff[0]);
       DPRINTF("\nRelay-1 Button %d, state %d\n", nvm_data_buff[1], relay_states[0]);
       DPRINTF("\nRelay-2 Button %d, state %d\n", nvm_data_buff[2], relay_states[1]);
       DPRINTF("\nRelay-3 Button %d, state %d\n", nvm_data_buff[3], relay_states[2]);
       DPRINTF("\nRelay-4 Button %d, state %d\n", nvm_data_buff[4], relay_states[3]);
       DPRINTF("last_relay_state: %d,%d,%d,%d\n",
               last_relay_state[0], last_relay_state[1],
               last_relay_state[2], last_relay_state[3]);
   }
  return AppData;
}
...
uint16_t GetConfigurationParameterValue(uint16_t parameterNumber)
{
  ...
  {
//    case 1:
//      return ApplicationData.rotary_ep;
//    break;
//
//    case 2:
//      return ApplicationData.open_close1_time;
//    break;
//
//    case 3:
//      return ApplicationData.open_close2_time;
//    break;
//
//    case 4:
//      return ApplicationData.open_close3_time;
//    break;
//
//    case 5:
//      return ApplicationData.ep1_current_level;
//    break;
//
//    case 6:
//      return ApplicationData.ep1_current_level;
